...
class RestaurantServiceTest {
    ...
    public void searching_for_existing_restaurant_should_return_expected_restaurant_object()  {
        ...
        Restaurant restaurantReturned = null;
        try {
            restaurantReturned = service.findRestaurantByName("Amelie's cafe");
        } catch (RestaurantNotFoundException e) {
            fail("should not throw exception");
        }
        assertEquals(restaurant, restaurantReturned);
    }

    //You may watch the video by Muthukumaran on how to write exceptions in Course 3: Testing and Version control: Optional content
    @Test
    public void searching_for_non_existing_restaurant_should_throw_exception() {
        assertThrows(RestaurantNotFoundException.class,()->service.findRestaurantByName("No Name Restaurant"));
    }
    //<<<<<<<<<<<<<<<<<<<<SEARCHING>>>>>>>>>>>>>>>>>>>>>>>>>>
